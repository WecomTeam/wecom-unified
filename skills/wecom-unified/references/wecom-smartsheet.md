...
管理企业微信**智能表格**（`/smartsheet/*`，doc_type=10）的创建，以及子表、字段/列、记录的增删改查。智能表格接口支持通过 `docid` 或 `url` 二选一定位。

> ⚠️ **表格 ≠ 智能表格**：二者是不同品类（`/sheet/` vs `/smartsheet/`）。需要结构化读写「子表 / 字段 / 记录」的场景走本文件；仅需读取在线表格内容请见 [wecom-sheet.md](wecom-sheet.md)。
...
| `https://doc.weixin.qq.com/smartsheet/*` | **智能表格**（doc_type=10） | 本文件 |
| `https://doc.weixin.qq.com/smartpage/*` | **智能文档**（原名智能主页） | [wecom-smartpage.md](wecom-smartpage.md) |

**判断规则**：URL 路径以 `/smartsheet/*` 开头 → 智能表格 → 使用本文件接口。
...
新建智能表格（doc_type=10）。创建成功返回 url 和 docid。
...
- 创建普通文档（doc_type=3）请见 [wecom-doc.md](wecom-doc.md)。

参见 [API 详情](wecom-doc-create-doc.md)。

### get_doc_content

获取**智能表格**的完整内容数据，统一以 Markdown 格式返回（用于整表内容浏览）。采用**异步轮询机制**：首次调用无需传 `task_id`，接口返回 `task_id`；若 `task_done` 为 false，需携带该 `task_id` 再次调用，直到 `task_done` 为 true 时返回完整内容。

> 适用 URL：`/smartsheet/*`。若需结构化获取记录（含字段类型），请使用 `smartsheet_get_records`。
...
- 通过 URL 读取智能表格：
```bash
wecom-cli doc get_doc_content '{"url": "https://doc.weixin.qq.com/smartsheet/xxx", "type": 2}'
```
...
wecom-cli doc create_doc '{"doc_type": 10, "doc_name": "任务跟踪表"}'
```
   保存返回的 docid（默认已含一个子表）
2. **整表内容浏览**（Markdown 形式） →
```bash
wecom-cli doc get_doc_content '{"docid": "DOCID", "type": 2}'
```
